...
const POPULAR_HUBS = [
  ...
  { name: "Bastar", lat: 19.076, lng: 82.0298 },
...
export default function Header({
  ...
  return (
    <>
      <header className="relative z-20 flex flex-col gap-2 px-4 md:px-8 py-3 border-b border-white/5 bg-black/50 backdrop-blur-md">
        {/* Main Header Bar */}
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {/* Vernacular Language Picker, Live Auto-Sync Status & User Profile */}
          <div className="flex items-center gap-2 order-2 sm:order-4">
            {/* Live Auto-Refresh Engine Indicator */}
            {onManualRefresh && (
              <button
                onClick={onManualRefresh}
                disabled={isRefreshing}
                title="Live satellite auto-refresh active. Click to sync fresh observation data now."
                className="hidden lg:flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-gray-950/80 border border-white/10 hover:border-yellow-400/40 text-[10px] font-mono text-gray-300 hover:text-white transition cursor-pointer"
              >
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping shrink-0" />
                <span className="text-emerald-400 font-bold">Auto-Sync</span>
                <span className="text-gray-400">• {lastSyncedSecondsAgo}s ago</span>
        ...
        {/* Quick Fast-Pill District / Metro Switcher Row */}
        <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none py-0.5 text-[10px] font-mono text-gray-400">
          <span className="text-yellow-400/80 font-bold shrink-0 uppercase tracking-wider mr-1">
            ⚡ Quick Hubs:
          </span>
          {POPULAR_HUBS.map((hub) => (
            <button
              key={hub.name}
              onClick={() => onSelectSearchCity(hub.name, { lat: hub.lat, lng: hub.lng })}
              className={`px-2.5 py-0.5 rounded-full border transition cursor-pointer shrink-0 ${
                currentCity?.toLowerCase() === hub.name.toLowerCase()
                  ? "bg-yellow-400 text-gray-950 font-extrabold border-yellow-400 shadow-sm"
                  : "bg-black/60 border-white/10 hover:border-yellow-400/40 text-gray-300 hover:text-white"
              }`}
            >
              {hub.name}
            </button>
